...
def init(*args, **kwargs):
    ...
    if get_response(msg, lambda inp: inp == 'yes', False):
        ...
        if get_response(msg, lambda inp: inp == 'yes', False):
            effect = PhotoEffect.objects.get_or_create(name='Enhance Thumbnail', description="Increases sharpness and contrast. Works well for smaller image sizes such as thumbnails.", contrast=1.2, sharpness=1.3)[0]
    ...
    if get_response(msg, lambda inp: inp == 'yes', False):
        create_photosize('thumbnail', width=100, height=75)
        ...
        if get_response(msg, lambda inp: inp == 'yes', False):
            effect = PhotoEffect.objects.get_or_create(name='Display Reflection', description="Generates a reflection with a white background", reflection_size=0.4)[0]

...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for a in Article.objects.all():
            if a not in a.auteur.get_profile().articles.all():
                a.auteur.get_profile().articles.add(a)
        for s in Sortie.objects.all():
            if s not in s.auteur.get_profile().sorties.all():
                s.auteur.get_profile().sorties.add(s)
            
        for comment in Comment.objects.all():
            for s in comment.sortie_comments_set.all():
                if s not in comment.auteur.get_profile().sorties.all():
                    comment.auteur.get_profile().sorties.add(s)
        
            for a in comment.article_comments_set.all():
                if a not in comment.auteur.get_profile().articles.all():
                    comment.auteur.get_profile().articles.add(a)
